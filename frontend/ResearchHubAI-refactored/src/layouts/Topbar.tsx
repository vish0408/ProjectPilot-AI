...
export default function Topbar({ nav, onMenu, showSearch = true }: { nav: typeof STUDENT_NAV; onMenu:()=>void; showSearch?: boolean }) {
  const { user, screen, setScreen, theme, setTheme, logout } = useApp();
  const label = nav.find(n=>n.id===screen)?.label || "Dashboard";
  const rc: Record<Role,string> = { student:"text-blue-600", guide:"text-indigo-600", admin:"text-slate-600", hod:"text-cyan-600", superadmin:"text-red-600", collegeadmin:"text-purple-600" };
  const rl: Record<Role,string> = { student:"PhD Student", guide:"Research Guide", admin:"Administrator", hod:"HOD", superadmin:"Super Admin", collegeadmin:"College Admin" };
  return (
