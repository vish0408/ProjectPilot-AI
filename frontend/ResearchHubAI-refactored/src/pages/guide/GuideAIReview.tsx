...
export default function GuideAIReview() {
  const [msgs, setMsgs] = useState<Msg[]>([{role:"assistant",text:"Hello Dr. Mehta! I can help you review student submissions faster — summarize chapters, flag issues, check for plagiarism patterns, and generate feedback. Which student's chapter would you like me to analyze?",time:"10:00 AM"}]);
  ...
  const send = () => {
    ...
    setTimeout(()=>{
      setTyping(false);
      setMsgs(p=>[...p,{role:"assistant",text:`**Analysis:** ${q}\n\n**Quality Score: 7.8/10**\n\n**Strengths:**\n- Well-structured methodology\n- Good use of recent literature (87% from 2021+)\n- Clear problem formulation\n\n**Issues:**\n- Section 3.4 lacks quantitative evaluation metrics\n- 2 citations appear incomplete\n- Passive voice overused in sections 3.1–3.2\n\n**Suggested Feedback:**\nAdd precision, recall, and F1-score tables in Section 3.4. Revise citations [8] and [15]. Restructure Section 3.2 conclusion to active voice.`,time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}]);
    },1800);
  };
  ...
  return (
    <div className="flex h-[calc(100vh-9rem)] gap-5">
      ...
      <div className="flex-1 flex flex-col bg-card border border-border rounded-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5 scrollbar-hide">
          {msgs.map((m,i)=>(
            <div key={i} className={`flex gap-3 ${m.role==="user"?"flex-row-reverse":""}`}>
              {m.role==="assistant"?<div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-xl flex items-center justify-center flex-shrink-0"><Brain className="w-4 h-4 text-white"/></div>:<Avatar name="Rajesh Mehta" size="sm"/>}
        ...
        <div className="p-4 border-t border-border"><div className="flex gap-2"><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Analyze chapter, generate feedback..." className="flex-1 bg-muted border border-border rounded-xl px-4 py-3 text-sm outline-none focus:border-primary"/><button onClick={send} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl"><Send className="w-4 h-4"/></button></div></div>
      </div>
    </div>
  );
}
