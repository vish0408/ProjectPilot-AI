  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{[{l:"Name",v:user?.name},{l:"Email",v:user?.email},{l:"Role",v:"College Admin"},{l:"Department",v:user?.dept},{l:"Institution",v:user?.institution}].map((f,i)=><div key={i}><label className="text-xs font-bold text-muted-foreground uppercase tracking-wide">{f.l}</label><p className="text-sm font-medium text-foreground mt-1">{f.v || "-"}</p></div>)}</div>
      </Card>
    </div>
  );
}
