...
export interface StudentListQuery extends PagedRequest {
  search?: string;
  sortBy?: string;
  filterStatus?: string;
}
...
export class HodService {
  ...
  // Students
  async getStudents(query: StudentListQuery = {}): Promise<PagedResponse<HodStudentSummary>> {
    const res = await apiClient.get<PagedResponse<HodStudentSummary>>("/hod/students", { params: query });
  ...
  async getStudentDetail(studentUserId: string): Promise<StudentDetail> {
    ...
    if (!res.success || !res.data) throw new Error(res.message || "Failed to get student details");
    return res.data;
  }

  async assignGuideToStudent(studentUserId: string, guideId: string, remarks?: string): Promise<void> {
  ...
  async getGuideDetail(guideUserId: string): Promise<GuideDetail> {
    ...
    if (!res.success || !res.data) throw new Error(res.message || "Failed to get guide details");
    return res.data;
  }
