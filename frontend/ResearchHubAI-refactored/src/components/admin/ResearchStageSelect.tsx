...
import type { ResearchStageResponse } from "../../types/Admin";

interface ResearchStageSelectProps {
  value: string;
...
export default function ResearchStageSelect({
  value,
  onChange,
...
}: ResearchStageSelectProps) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);
  const [stages, setStages] = useState<ResearchStageResponse[]>([]);
  ...
  useEffect(() => {
    if (disabled) return;
    setLoading(true);
    adminService.getResearchStages()
      .then((data) => {
        const active = (data || []).filter((s) => s.isActive);
        setStages(active.length > 0 ? active : data || []);
      })
      .catch(() => setStages([]))
      ...
      {open && !disabled && (
        <div className="absolute z-20 mt-1 w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-lg overflow-hidden">
          ...
          <div className="max-h-48 overflow-y-auto">
            ...
            {filtered.length === 0 ? (
              <p className="px-3 py-3 text-xs text-slate-400 text-center">No research stages found</p>
            ) : (
              filtered.map((s) => (
                <button
                  key={s.id}
